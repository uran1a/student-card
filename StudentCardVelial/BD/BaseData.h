...
public:
	...
	void Insert(String^ Name, String^ Surname, String^ Middlename, String^ Birthday, String^ PointEGE, String^ PhotoStudent, String^ EducationalForm, String^ PhoneNumber) {
		try {
			//Подключение в БД
			ConnectToBD();

			String^ cmdText = "INSERT INTO dbo.TABLE_STUDENTS(Name, Surname, Middlename, Entrant, Title_Group, Birthday, Point_EGE, Stipendiya, Year_Enrollment, Photo_Student, Specialization, Educational_Form, Number_Kurc, Phone_Number) VALUES(@Name, @Surname, @Middlename, @Entrant, @Title_Group, @Birthday, @PointEGE, @Stipendiya, @Year_Enrollment, @Photo_Student, @Specialization, @Educational_Form, @Number_Kurc, @Phone_Number)";
			SqlCommand^ cmd = gcnew SqlCommand(cmdText, conn);
			...
			cmd->Parameters->AddWithValue("@PointEGE", PointEGE);
			cmd->Parameters->AddWithValue("@Year_Enrollment", "");
			cmd->Parameters->AddWithValue("@Photo_Student", PhotoStudent);
			cmd->Parameters->AddWithValue("@Specialization", "");
			cmd->Parameters->AddWithValue("@Educational_Form", EducationalForm);
	...
	List<Student^>^ FillListViewStudent(String^ NameGroup) {
		try {
			...
			while (reader->Read()) {
				...
				//student->Year_Enrollment = Convert::ToInt32(reader["Year_Enrollment"]->ToString());
				student->Year_Enrollment = (reader["Year_Enrollment"]->ToString());
				Console::WriteLine("{0}", reader["Entrant"]->ToString());
				list->Add(student);
				}
			}
	...
	void Update(int ID, int Entrant, String^ PathGroup) {
		try {
			//Подключение в БД
			ConnectToBD();

			String^ cmdText = "UPDATE dbo.TABLE_STUDENTS SET Entrant = @Entrant, Title_Group = @Title_Group  WHERE ID = @ID";
			SqlCommand^ cmd = gcnew SqlCommand(cmdText, conn);
